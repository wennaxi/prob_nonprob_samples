...
p_nonprob = exp(- 10 + 1 * population_nonprob$X1 - 2 * population_nonprob$X2 + 3 * nonprob.x3.2 + 4 * nonprob.x3.3 - 5 * nonprob.x4.2 - 6 * nonprob.x4.3) / 
  (exp(- 10 + 1 * population_nonprob$X1 - 2 * population_nonprob$X2 + 3 * nonprob.x3.2 + 4 * nonprob.x3.3 - 5 * nonprob.x4.2 - 6 * nonprob.x4.3) + 1)
...
if (wt == 1) {
  ...
  sample_prob = merge(sample_prob, w1, by = 'S2')[, -15]
  # dim(sample_all)
  # head(sample_all)
...
if (ps == "No") {
  ...
  dstrats = svydesign(id = ~1, strata = ~S2, weights = ~weight, fpc = ~fpc2, data = sample_prob)
  #summary(dstrats)
} else if (ps == "External") {
  #####3ci1 joint post-stratification (external info)
  dstrat = svydesign(id = ~1, strata = ~S2, weights = ~weight, fpc = ~fpc2, data = sample_prob)
  ...
  sample_prob$weight = weights(dstratps)
  #####Weight Normalization#####
  sample_prob$weight_norm = (N / sum(sample_prob$weight)) * sample_prob$weight
  sample_prob$weight = sample_prob$weight_norm
  sample_prob = sample_prob[, -16]
